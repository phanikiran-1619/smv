...
import { UserCheck, Save, RotateCcw, Upload, FileDown } from 'lucide-react';
import * as XLSX from "xlsx";

const AttenderRegistrationPage = () => {
  ...
  const [selectedAttenderForUpdate, setSelectedAttenderForUpdate] = useState("");
  const [loadingStates, setLoadingStates] = useState({
    attenders: false,
    attenderDetails: false,
  });
  ...
  useEffect(() => {
    const initializePage = async () => {
      ...
      if (isUpdateMode) {
        await fetchAttenders();
      }
    };
    initializePage();
  }, [isUpdateMode]);

  // Fetch attenders for update mode dropdown
  const fetchAttenders = async () => {
    try {
      setLoadingStates(prev => ({ ...prev, attenders: true }));
      const token = getToken();
      if (!token) return;
      
      const response = await fetch(`${API_BASE_URL}/attender`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      
      if (response.ok) {
        const data = await response.json();
        setAttenders(Array.isArray(data) ? data : []);
      }
    } catch (error) {
      console.error('Error fetching attenders:', error);
    } finally {
      setLoadingStates(prev => ({ ...prev, attenders: false }));
    }
  };

  // Fetch individual attender details for auto-fill
  const fetchAttenderDetails = async (smAttenderId) => {
    try {
      setLoadingStates(prev => ({ ...prev, attenderDetails: true }));
      const token = getToken();
      if (!token) return;
      
      const response = await fetch(`${API_BASE_URL}/attender/by-attenderId?smAttenderId=${smAttenderId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      setAlertMessage("Failed to fetch attender details");
    } finally {
      setLoadingStates(prev => ({ ...prev, attenderDetails: false }));
    }
  };
  ...
  const resetForm = (newUpdateMode = false) => {
    ...
    setExcelError(null);
    setErrors({});
    setSelectedAttenderForUpdate("");
  };

  const handleAttenderSelection = async (smAttenderId) => {
    setSelectedAttenderForUpdate(smAttenderId);
    if (smAttenderId) {
      await fetchAttenderDetails(smAttenderId);
    }
  };

  const checkExistingAttender = async (field, value) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/attender/check?${field}=${value}`,
        {
  ...
  if (loadingStates.attenderDetails) {
    return (
          ...
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-8">
              <h1 className={`text-4xl font-bold ${themeClasses.title} mb-2 flex items-center justify-center`}>
                ...
                Enhanced Attender Registration
              </h1>
              <p className={themeClasses.subtitle}>Loading attender data...</p>
            </div>
            <SkeletonForm />
          </div>
        </div>
      ...
      <div className="pt-24 px-4 pb-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <p className={themeClasses.subtitle}>Advanced attender registration with comprehensive validation and Excel support</p>
          </div>
          ...
          <Card className={`${themeClasses.card} p-8 rounded-2xl`}>
            {/* Attender Selection Dropdown for Update Mode */}
            {isUpdateMode && (
              <div className={`mb-6 p-4 ${theme === 'dark' ? 'bg-blue-500/10 border-blue-500' : 'bg-blue-50 border-blue-300'} border rounded-xl`}>
                <Label className={`${theme === 'dark' ? 'text-blue-300' : 'text-blue-700'} font-medium mb-2 block`}>
                  Select Attender to Update
                </Label>
                {loadingStates.attenders ? (
                  <SkeletonLoader />
                ) : (
                  <SearchableSelect
                    options={attenders.map(attender => ({
                      value: attender.smAttenderId,
                      label: `${attender.firstName} ${attender.lastName} (${attender.smAttenderId}) - ${attender.user?.username}`
                    }))}
                    value={selectedAttenderForUpdate}
                    onValueChange={handleAttenderSelection}
                    placeholder="Search and select an attender..."
                    searchPlaceholder="Type to search attenders..."
                    className={themeClasses.input}
                  />
                )}
                <p className={`${theme === 'dark' ? 'text-blue-300' : 'text-blue-600'} text-sm mt-2`}>
                  Select an attender from the dropdown to auto-fill the form with current data
                </p>
              </div>
            )}
            ...
            <form
              onSubmit={async (event) => {
                ...
                if (!isUpdateMode) {
                  if (!(await checkExistingAttender("username", formData.username))) {
                    setIsSubmitting(false);
                    return;
                  }
                  if (!(await checkExistingAttender("email", formData.email))) {
                    setIsSubmitting(false);
                    return;
                  }
                }
            ...
            >
              <div className={`text-2xl font-bold text-center ${theme === 'dark' ? 'text-white' : 'text-gray-900'} mb-4`}>
                Attender {isUpdateMode ? "Update" : "Registration"}
              </div>
...
export default AttenderRegistrationPage;