...
import { Car, Save, RotateCcw, FileDown } from 'lucide-react';
...
const DriverRegistrationPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.smDriverId || !formData.firstName || !formData.lastName || !formData.phone) {
    ...
    try {
      ...
      const url = mode === 'register' 
        ? `${API_BASE_URL}/driver`
        : `${API_BASE_URL}/driver/${formData.smDriverId}`;
      ...
      setSuccess(`Driver ${mode === 'register' ? 'registered' : 'updated'} successfully!`);
      ...
      setError(error.response?.data?.message || `Failed to ${mode} driver`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="pt-24 px-4 pb-8">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-300">Register and manage driver information</p>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-white text-sm font-medium mb-2">
                    SM Driver ID <span className="text-red-400">*</span>
                  </label>
                  <Input
                    type="text"
                    value={formData.smDriverId}
                    onChange={(e) => handleInputChange('smDriverId', e.target.value)}
                ...
                >
                  {loading ? (
                    ...
                    <span className="flex items-center">
                      ...
                      {mode === 'register' ? 'Register Driver' : 'Update Driver'}
                    </span>
                  )}
                </Button>
